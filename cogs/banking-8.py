...
# Abuja's employee role is named after FCT (the Federal Capital Territory
# that administers it) rather than "Abuja" -- e.g. "FCT Employee", matching
# how the FCT Minister etc. are referred to in-universe. Everywhere else in
# this codebase (players.state, config.STATES, the `locations` table) still
# calls the state "Abuja", so this map translates between the two.
STATE_ROLE_LABEL = {
    "Abuja": "FCT",
    "Lagos": "Lagos",
    "Delta": "Delta",
...
def is_bank_staff():
    ...
    "Bank Staff" and "{state} Employee" are two separate roles that must
    BOTH be held -- a Lagos-employed Bank Staff member does not count as
    Delta's Bank Staff just by holding the generic "Bank Staff" role."""
    ...
    async def predicate(ctx: commands.Context) -> bool:
        ...
        has_job_role = await _member_has_role(ctx.author, "Bank Staff")
        has_employee_role = await _member_has_role(ctx.author, f"{label} Employee")
        return has_job_role and has_employee_role
