...
class Cars(commands.Cog):
    ...
    async def _swap_state_role(self, member: discord.Member, old_state: str | None, new_state: str) -> None:
        ...
        if old_state and old_state.upper() != new_state.upper():
            old_role_id = await database.get_role_id(old_state.title())
            old_role = member.guild.get_role(old_role_id) if old_role_id else None
            if old_role is not None and old_role in member.roles:
                await member.remove_roles(old_role)

        new_role_id = await database.get_role_id(new_state.title())
        new_role = member.guild.get_role(new_role_id) if new_role_id else None
